...
<template>
    <v-container dense density="compact">
        <v-row dense density="compact">
            <v-col :cols="12" :sm="6" dense density="compact">
                <v-card-title>
                    {{ meetup.name }}
                </v-card-title>
                <v-list density="compact">
                    ...
                    <v-list-item density="compact">
                        ...
                        <template #title>
                            <v-tooltip location="bottom center" :open-on-click="false">
                                <template #activator="{ props }">
                                    <NuxtLink :to="mapURL" target="_blank" external v-bind="props">
                                        {{ venue.name }}
                                    </NuxtLink>
                                </template>
                                {{ venue.name }}
                        ...
                        <template #subtitle>
                            <v-tooltip location="bottom center" :open-on-click="true">
                                <template #activator="{ props }">
                        ...
                        <template #title>
                            <template v-for="shuttle in shuttles">
                                <v-tooltip location="bottom center">
                                    <template #activator="{ props }">
                                        <NuxtLink to="javascript:void(0)" v-bind="props">
                                            {{ shuttle.name }}
                                        </NuxtLink>
                                    </template>
                                    {{ shuttle.name }}
                    ...
                    <v-list-item density="compact">
                        <template #prepend>
                            ...
                            <v-avatar :size="20" color="#02c755" style="filter: grayscale(1)">
                                <v-img
                                    src="https://firebasestorage.googleapis.com/v0/b/boyholic-badminton.appspot.com/o/assets%2Fimages%2Fline-88.png?alt=media&token=2ce753b7-0f30-4ddc-948f-1e9e45c392ec" />
                            </v-avatar>
