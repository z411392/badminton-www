...
const onSaveButtonClicked = async () => {
    loading.value = true
    const profileId = profile.value ? (await onUpdatingProfile(groupId, profile.value!.id, unref(mutation)))! : (await onCreatingProfile(groupId, unref(mutation)))!
...
<template>
    <v-dialog v-model="opened" width="auto" :persistent="true">
        <v-card :width="360" :loading="loading">
            <v-card-title>
